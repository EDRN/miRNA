#Embed a target plot into the existing stuff Scott Has.

#TargPlot<-makeTargetPlot(indf=Lab008_A_R03,key=Sample_Key_R03,correction=c(.348,.167,.485),expectation=data.frame(mix1=c(.25,.25,.5),mix2=c(.5,.25,.25)))

# Expectation : Call TargetPlot with the following:  [input df, site_platform]
makeTargetPlot<-function(lab,indf,correction=c(1,1,1),expectation=data.frame(mix1=c(.25,.25,.5),mix2=c(.5,.25,.25)),isLog=TRUE,metrics=FALSE){
  # Inputs:
...
targetblotse<-function(lab,interval1,interval2,tabular=FALSE){
  ...
  cdf <- do.call(rbind,
    ...
    g<-ggplot(data=subset(aa,Lab==lab))+
      # geom_point(data=data.frame(expectation,Tissue=c("Brain","Liver","Placenta")),aes(mix1,mix2,col=Tissue),size=2)+
      stat_ellipse(data=subset(ellipse,Lab==lab),aes(x=value,y=value2,col=Tissue,fill=Tissue),geom="polygon",alpha=0.75)+
      scale_fill_manual(values = c("blue","yellow", "red"))+
      # ylim(-0.25,1) +
      # xlim(-0.25,1) +
      scale_y_continuous(limits=c(-0.15,0.9),breaks=c(0,0.25,0.5,0.75)
                         ,minor_breaks=c(0.05,0.1,0.15,0.2,0.3,0.35,0.4,0.45,0.55,0.6,0.65,0.7)
                         ) +
      scale_x_continuous(limits=c(-0.15,0.9),breaks=c(0,0.25,0.5,0.75)
                         ,minor_breaks=c(0.05,0.1,0.15,0.2,0.3,0.35,0.4,0.45,0.55,0.6,0.65,0.7)
                         ) +
      labs(x="Input Est. Mix1", y="Input Est. Mix2", title = paste(" "))+
      theme(legend.position="none")+
      coord_fixed()+geom_path(data=cdf,alpha=0.35,aes(group=factor(r),x=x,y=y,col=r))+
      scale_color_manual(values = c("black","blue","blue","black","orange","orange","black","red","red"))+
      geom_segment(aes(x=pointdata["Liver"][1,],xend=expectation["mix1"][2,],y=pointdata["Liver"][2,],yend=expectation["mix2"][2,],col="Liver"), linetype = "solid", color="orange")+
      geom_segment(aes(x=pointdata["Brain"][1,],xend=expectation["mix1"][1,],y=pointdata["Brain"][2,],yend=expectation["mix2"][1,],col="Brain"), linetype = "solid", color="blue")+
      geom_segment(aes(x=pointdata["Placenta"][1,],xend=expectation["mix1"][3,],y=pointdata["Placenta"][2,],yend=expectation["mix2"][3,],col="Placenta"), linetype = "solid", color="red")
    return(g)
}
