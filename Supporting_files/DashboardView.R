...
# 
# if (length(args)<10) {
#   stop("Eight arguments (Dataset, TSI_Br, TSI_Lv, TSI_Pl, BP_sim, BW, FC, Prop) must be supplied (e.g. Lab008, NGS004, NIST05, 10, 10, 10, 0.075, 2, 1.5, 0.95)\n", call.=FALSE)
# }
# 
# Dataset  <- args[1]
# TSI_Br   <- args[2]
...
#######################################
ShinyDB6P <- function (Dataset, TSI_Br, TSI_Lv, TSI_Pl, BP_sim, BW, FC, Prop) {

# Parse Dataset Name
  DatasetName      <- as.vector(Dataset[1,1])
  DatasetNameParts <- as.vector(unlist(strsplit(DatasetName, "_", fixed = TRUE)))
  
  Site     <- DatasetNameParts[1]
  Protocol <- DatasetNameParts[2]
  Sample   <- sub(".txt","",DatasetNameParts[3])
  ...
  source("Supporting_files/LowerLimit.R")
  source("Supporting_files/TargetPlot.R")
  source("Supporting_files/TargetBias.R")

  # Import specific files by Sample#
  Lab_df <- read.delim(paste("Benchmarking_datasets/",Site,"_",Protocol,"_",Sample,".txt",sep=""))
  # Lab_df <- read.csv(paste(Site,"_",Protocol,"_",Sample,".csv",sep=""))

  # Import Sample Key
  Sample_Key <- read.delim(paste("Sample_keys/",Sample,"_Key",".txt",sep=""), skip=21)

  # Import annotation files
  miRbase21_mature_wHC <- read.delim("Supporting_files/miRbase21_human_mature_wHC.txt")
  ...
  TSI_Class <- ifelse(TSI_B >=TSI_Br , "10X_Brain",
                      ifelse(TSI_L >=TSI_Lv, "1_to_1", # 10X_Liver incorporated into 1_to_1 TSI_Class
                             ifelse(TSI_P >=TSI_Pl, "10X_Placenta",
                                    ifelse(abs(log2(LinB/LinP/LinL)) ==0.0, "NS",
                                           ifelse(abs(log2(LinB/LinP)) <=BP_sim, "1_to_1", "NS")))))
  ...
  PredictedPlot <- ggplot(Mix1_v_Mix2, aes(A_ModM1M2, M_ModM1M2, fill=TSI_Class, shape=TSI_Class, alpha=TSI_Class))  +
    ...
    ylim(-2.5,2.5) +
    scale_x_continuous(limits=c(-2.5,22.5),breaks=c(0,5,10,15,20)) +
    theme(legend.position = "none") +
    labs(x = "Average Log2 Signal",
         y = "Predicted Log2 Ratio",
         title = " ")
  ...
  # Distribution calculations for DFPrPlot and DFPrBoxPlot
  bias <- median(Mix1_v_Mix2$DFP, na.rm=TRUE)
  iqr  <- (IQR(Mix1_v_Mix2$DFP, na.rm=TRUE))
  Q1   <- quantile(Mix1_v_Mix2$DFP, na.rm=TRUE, 0.25)
  Q2   <- quantile(Mix1_v_Mix2$DFP, na.rm=TRUE, 0.5)
  Q3   <- quantile(Mix1_v_Mix2$DFP, na.rm=TRUE, 0.75)

  # defaults used for MS (allow user input in later version)
  bw   <- BW
  tol  <- log2(FC)      # log2 of 1.5X fold-change - up or down  (allow user input in later version)
  prop <- Prop
  ...
  DFPrPlot <- ggplot(Mix1_v_Mix2, aes((A+A_ModM1M2)/2, M-M_ModM1M2, fill=TSI_Class, shape=TSI_Class, alpha=TSI_Class))  +
    ...
    scale_shape_manual(values = c(21,21,21,1)) +
    scale_alpha_manual(values = c(1.0,1.0,1.0,0.3)) +
    geom_hline(aes(yintercept = Q2), color="black", size=.75, linetype=1) +
    geom_hline(aes(yintercept = Q3), color="black", size=.75, linetype=2) +
    geom_hline(aes(yintercept = Q1), color="black", size=.75, linetype=2) +
    ylim(-3,3) +
    scale_x_continuous(limits=c(-2.5,22.5),breaks=c(0,5,10,15,20)) +
    theme(legend.position = "none") +
    labs(x = "Average Log2 Signal",
         y = "Log2 Ratio Deviation",
         title = " ")
  ...
  ggsave(file=paste("PDF_output/",Site,"_",Protocol,"_",Sample,".pdf", sep=""), g, width = 7.5, height = 10)
  
}
###########################
# Dashboard6P(Site, Protocol, Sample, TSI_Br, TSI_Lv, TSI_Pl, BP_sim, BW, FC, Prop)
